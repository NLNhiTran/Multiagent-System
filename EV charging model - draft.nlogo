...
to move
  forward 1
  set battery battery - movement-cost ;; reduce battery by the cost of movement
end

to refuel
  if battery < 5 and (member? patch-here [ neighbors4 ] of charging-station) [
    set goal charging-station
  ]
end

to check if-out-of-battery
  if battery < 0
     speed = 0
  ]
end
